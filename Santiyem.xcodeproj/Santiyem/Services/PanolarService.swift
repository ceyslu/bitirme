...
final class PanolarService {
    ...
    func fetchDashboardData(filtre: PanoFiltre) async throws -> PanoOzet {
        async let jobsTask = fetchJobs()
        async let fuelTask = FuelExpenseService.shared.fetchFuelExpenses()
        async let maintenanceTask = BakimGideriService.shared.fetchMaintenanceExpenses()
    ...
    private func fetchJobs() async throws -> [PanoIsKaydi] {
