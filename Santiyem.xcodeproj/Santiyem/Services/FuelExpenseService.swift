...
struct YakitGideriIstek: Codable {
    let machineId: Int
    let cost: Double
    let liters: Double
...
final class FuelExpenseService {
    ...
    func fetchFuelExpenses() async throws -> [YakitGideri] {
        ...
        if 200 ... 299 ~= httpResponse.statusCode {
            ...
            throw NSError(domain: "", code: httpResponse.statusCode, userInfo: [
                NSLocalizedDescriptionKey: apiError?.error ?? "Mazot giderleri alınamadı"
            ])
        }
    }
    ...
    func createFuelExpense(
        ...
        if 200 ... 299 ~= httpResponse.statusCode {
            ...
            let apiError = try? JSONDecoder().decode(APIErrorResponse.self, from: data)
            throw NSError(domain: "", code: httpResponse.statusCode, userInfo: [
                NSLocalizedDescriptionKey: apiError?.error ?? "Mazot gideri eklenemedi"
            ])
        }
    }
}
